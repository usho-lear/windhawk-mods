...
InjectionTarget ResolveInjectionTarget(FrameworkElement const& root) {
    ...
    auto trayPanel = trayFrame ? trayFrame.try_as<controls::Panel>() : nullptr;
    if (trayPanel) {
        int col = -1;
        ...
        if (trayGrid) {
            ...
            if (g_settings.position == L"tray_left") {
                ...
                col = static_cast<int>(trayPanel.Children().Size());
            } else if (g_settings.position == L"tray_before_clock") {
                auto clock = FindChildByName(trayPanel, L"NotificationCenterButton");
                if (!clock) {
                    clock = FindChildByName(root, L"NotificationCenterButton");
                }
                if (clock) {
                ...
                auto showDesktop = FindChildByName(trayPanel, L"ShowDesktopStack");
                if (!showDesktop) {
                    showDesktop = FindChildByName(root, L"ShowDesktopStack");
                }
                if (showDesktop) {
                    auto children = trayPanel.Children();
                    for (uint32_t i = 0; i < children.Size(); ++i) {
                        if (children.GetAt(i) == showDesktop) {
...
Grid BuildIslandGrid(bool isStackPanel = false) {
    double layoutHeight = g_layout.compactHeight;
    double verticalMargin = 0.0;

    if (isStackPanel) {
        double targetLayoutHeight = 32.0; // standard tray height
        if (layoutHeight > targetLayoutHeight) {
            verticalMargin = (targetLayoutHeight - layoutHeight) / 2.0; // e.g. -4.0
            layoutHeight = targetLayoutHeight;
        }
    }
    ...
    wrapper.MinHeight(layoutHeight);
    ...
    background.Margin({0, verticalMargin, 0, verticalMargin});
    controls::Canvas::SetZIndex(background, 0);

    Grid content;
    content.Name(L"Island_Content");
    content.Margin({g_layout.contentMarginX, g_layout.contentMarginY + verticalMargin,
                    g_layout.contentMarginX, g_layout.contentMarginY + verticalMargin});
...
void RemoveIslandGrid() {
    ...
    try {
        if (auto targetPanel = g_injectionParent.try_as<controls::Panel>()) {
            auto children = targetPanel.Children();
            for (uint32_t i = 0; i < children.Size(); ++i) {
            ...
            if (targetGrid && g_playerColumn >= 0 &&
...
bool InjectIslandGrid() {
    ...
    try {
        ...
        UpdateRuntimeLayout(hwnd, root, target.panel);
        bool isStackPanel = (target.panel.try_as<controls::StackPanel>() != nullptr);
        Grid island = BuildIslandGrid(isStackPanel);
        ...
        if (target.overlay) {
            ...
            target.panel.Children().Append(island);
            g_playerColumn = -1;
        } else {
            ...
            if (targetGrid) {
                ...
                if (col >= static_cast<int>(targetGrid.ColumnDefinitions().Size())) {
                    targetGrid.ColumnDefinitions().Append(colDef);
                } else {
                    targetGrid.ColumnDefinitions().InsertAt(col, colDef);
                ...
                targetGrid.Children().Append(island);
                g_playerColumn = col;
            } else {
                // For StackPanel, just insert at the resolved index
                int col = Clamp(target.column, 0, static_cast<int>(target.panel.Children().Size()));
                target.panel.Children().InsertAt(col, island);
                g_playerColumn = col;
            }
        }
        ...
        StartTaskbarLayoutMonitor(root, target.panel);
        ApplyExpandedState();
        UpdatePlayerContents();
        target.panel.UpdateLayout();
        RefreshCompactTextHostClip(false);
